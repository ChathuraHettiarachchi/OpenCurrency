...
/**
 * CurrencyDto will used to get currency names from API
 */
...
/**
 * Convert API result to list since it's not an object array
 */
fun CurrencyDto.asList(): List<Currency> {
	val gson = Gson()
	val map: Map<*, *> = gson.fromJson(gson.toJson(this), MutableMap::class.java)

	val result: MutableList<Currency> = mutableListOf()
	map.forEach { (k, v) ->
		result.add(
			Currency(
				0,
